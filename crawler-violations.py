...
crawled_df = pd.DataFrame(columns=['id', 'compliance'])
try:
    with sync_playwright() as p:
        ...
        for i in range(0, df.shape[0]):
            ...
            if compliance_img is None:
                log(f"Compliance image not found for Provider {provider_id} at {url}")
                continue
            compliance = compliance_img.get_attribute('src').split('/')[-1].split('_FINAL.png')[0]
            crawled_df.loc[i, 'id'] = provider_id
            crawled_df.loc[i, 'compliance'] = compliance

            log(f"Successfully retrieved violations data for Provider {provider_id} at {url}")
except Exception as e:
    crawled_df.to_csv('data/crawled_data/scraped_compliance.csv', index=False)
    traceback.print_exc()
